...
impl AtomicRateLimiter {
    pub fn load(rate_to_ms_conversion: i32, consumed_tokens: i32, last_refill_time: i64) -> Self {
        Self {
            rate_to_ms_conversion,
            consumed_tokens: AtomicI32::new(consumed_tokens),
            last_refill_time: AtomicI64::new(last_refill_time),
        }
    }
    ...
    fn refill_token(&self, burst_size: i32, average_rate: i64, current_time_millis: i64) {
        ...
        if self
            .last_refill_time
            .compare_exchange(refill_time, new_refill_time, Relaxed, Relaxed)
            .is_ok()
        {
            loop {
                ...
                if self
                    .consumed_tokens
                    .compare_exchange(current_level, new_level, Relaxed, Relaxed)
                    .is_ok()
                {
                    return;
                }
    ...
    fn consume_token(&self, burst_size: i32) -> bool {
        loop {
            ...
            if self
                .consumed_tokens
                .compare_exchange(current_level, current_level + 1, Relaxed, Relaxed)
                .is_ok()
            {
                return true;
            }
    ...
    pub fn get_consumed_tokens(&self) -> i32 {
        self.consumed_tokens.load(Relaxed)
    }
